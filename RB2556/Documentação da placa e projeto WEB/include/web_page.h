...
// ====================================================================
// ROBOBUILDERS RB2556 (esp32_relay_30a_x1_v1.1) - PAINEL DE TESTE
// LED Onboard: GPIO 5 | Relé 30A: GPIO 16 | Botão Boot: GPIO 0 (IO0)
// ====================================================================
...
<head>
  ...
  <title>ROBOBUILDERS RB2556 - Teste de Placa</title>
  <style>
    :root {
      --bg: #0b0f19;
      --card-bg: #131b2e;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    .btn { cursor: pointer; border: none; padding: 10px 16px; border-radius: 8px; font-weight: 600; font-size: 0.88rem; transition: background 0.15s, transform 0.1s; display: inline-flex; align-items: center; justify-content: center; gap: 6px; min-height: 42px; user-select: none; }
...
<body>
  ...
  <header>
    <h1>ROBOBUILDERS RB2556</h1>
    <p class="sub">esp32_relay_30a_x1_v1.1 • Painel de Teste de Hardware</p>
    <div class="badge-bar">
      <span class="badge live">● Conectado (192.168.4.1)</span>
      <span class="badge">Wi-Fi: ROBOBUILDERS-RB2556</span>
      <span class="badge" id="uptimeBadge">Uptime: 0s</span>
    </div>
  </header>

  <div class="section-title">1. Teste Rápido dos Recursos Onboard</div>
  <div class="hero-grid">
    ...
    <!-- RELÉ DE POTÊNCIA 30A (GPIO 16) -->
    <div class="hero-card" id="cardRelay">
      <h3>Relé de Potência 30A</h3>
    ...
    <div class="modal">
      ...
      <label class="radio-option">
        ...
        <div>
          ...
          <div style="font-size:0.75rem; color:#94a3b8;">Permite ligar e desligar cargas ou relés</div>
        </div>
      </label>
  ...
  <footer>
    ROBOBUILDERS RB2556 (esp32_relay_30a_x1_v1.1) • Teste de Hardware de Fábrica
  </footer>

  <script>
    // Lista de GPIOs adicionais para teste (GPIO 5 = LED Onboard, GPIO 16 = Relé 30A, GPIO 0 = Botão Boot)
    const gpioList = [2, 4, 12, 13, 14, 15, 17, 18, 19, 21, 22, 23, 25, 26, 27, 32, 33, 34, 35];
    let pinConfigs = {};
        ...
        .then(function(data) {
          ...
          if (data.pins) {
            data.pins.forEach(function(p) {
              ...
              // Relé 30A (GPIO 16)
              if (p.pin === 16) {
                var cardRelay = document.getElementById('cardRelay');
                var pillRelay = document.getElementById('pillRelay');
                var btnRelay = document.getElementById('btnRelay');
                if (cardRelay && pillRelay && btnRelay) {
                  if (p.val === 1) {
                    cardRelay.className = 'hero-card active';
                    pillRelay.className = 'status-pill status-on';
                    pillRelay.innerText = 'LIGADO (FECHADO)';
                    btnRelay.className = 'btn btn-red';
                    btnRelay.innerText = 'Desligar Relé';
                  } else {
                    cardRelay.className = 'hero-card';
                    pillRelay.className = 'status-pill status-off';
                    pillRelay.innerText = 'DESLIGADO (ABERTO)';
                    btnRelay.className = 'btn btn-green';
                    btnRelay.innerText = 'Acionar Relé';
                  }
                }
              }
