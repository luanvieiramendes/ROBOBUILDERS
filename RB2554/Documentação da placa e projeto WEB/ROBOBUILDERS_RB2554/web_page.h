...
// ====================================================================
// ROBOBUILDERS RB2554 (ESP-32_RELAY_30A_X4_V1.1) - PAINEL DE TESTE
// LED Onboard: GPIO 5 | Relés 30A: GPIO 12, 13, 14, 15 | Botão Boot: GPIO 0 (IO0)
// ====================================================================
...
<head>
  ...
  <title>ROBOBUILDERS RB2554 - Teste de Placa (4x Relé 30A)</title>
  <style>
    :root {
      --bg: #0b0f19;
      --card-bg: #131b2e;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    .btn { cursor: pointer; border: none; padding: 10px 14px; border-radius: 8px; font-weight: 600; font-size: 0.85rem; transition: background 0.15s, transform 0.1s; display: inline-flex; align-items: center; justify-content: center; gap: 6px; min-height: 44px; min-width: 44px; user-select: none; flex: 1; }
    ...
    .btn-toggle { width: 100%; min-height: 44px; padding: 8px 4px; border-radius: 6px; border: 1px solid #334155; font-weight: 700; font-size: 0.82rem; cursor: pointer; transition: all 0.15s; }
    ...
    .input-badge { padding: 10px 4px; border-radius: 6px; font-weight: 700; font-size: 0.82rem; text-align: center; display: block; border: 1px solid #334155; min-height: 44px; display: flex; align-items: center; justify-content: center; }
...
<body>
  ...
  <header>
    <h1>ROBOBUILDERS RB2554</h1>
    <p class="sub">ESP-32_RELAY_30A_X4_V1.1 • Painel de Diagnóstico e Controle</p>
    <div class="badge-bar">
      <span class="badge live">● Conectado (192.168.4.1)</span>
      <span class="badge model">4x Relés 30A</span>
      <span class="badge">Wi-Fi: ROBOBUILDERS-RB2554</span>
      <span class="badge" id="uptimeBadge">Uptime: 0s</span>
    </div>
  </header>

  <div class="section-title">
    <span>1. Teste dos 4 Relés de Potência (30A) & Recursos Onboard</span>
  </div>

  <div class="bulk-bar">
    <button class="btn btn-green" onclick="setRelays(1)">⚡ Ligar Todos os Relés (1 a 4)</button>
  ...
  <div class="hero-grid">
    ...
    <!-- RELÉ 1 - 30A (GPIO 12) -->
    <div class="hero-card" id="cardRelay_12">
      <div>
        <h3>⚡ Relé 1 (30A)</h3>
        <span class="pin-tag">Pino Físico: <b>GPIO 12</b></span>
        <div id="pillRelay_12" class="status-pill status-off">DESLIGADO (ABERTO)</div>
      </div>
      <div class="btn-group">
        <button class="btn btn-green" id="btnRelay_12" onclick="togglePin(12)">Acionar Relé</button>
        <button class="btn btn-secondary" onclick="pulsePin(12, 1000)">Pulso 1s</button>
      </div>
    </div>

    <!-- RELÉ 2 - 30A (GPIO 13) -->
    <div class="hero-card" id="cardRelay_13">
      <div>
        <h3>⚡ Relé 2 (30A)</h3>
        <span class="pin-tag">Pino Físico: <b>GPIO 13</b></span>
        <div id="pillRelay_13" class="status-pill status-off">DESLIGADO (ABERTO)</div>
      </div>
      <div class="btn-group">
        <button class="btn btn-green" id="btnRelay_13" onclick="togglePin(13)">Acionar Relé</button>
        <button class="btn btn-secondary" onclick="pulsePin(13, 1000)">Pulso 1s</button>
      </div>
    </div>

    <!-- RELÉ 3 - 30A (GPIO 14) -->
    <div class="hero-card" id="cardRelay_14">
      <div>
        <h3>⚡ Relé 3 (30A)</h3>
        <span class="pin-tag">Pino Físico: <b>GPIO 14</b></span>
        <div id="pillRelay_14" class="status-pill status-off">DESLIGADO (ABERTO)</div>
      </div>
      <div class="btn-group">
        <button class="btn btn-green" id="btnRelay_14" onclick="togglePin(14)">Acionar Relé</button>
        <button class="btn btn-secondary" onclick="pulsePin(14, 1000)">Pulso 1s</button>
      </div>
    </div>

    <!-- RELÉ 4 - 30A (GPIO 15) -->
    <div class="hero-card" id="cardRelay_15">
      <div>
        <h3>⚡ Relé 4 (30A)</h3>
        <span class="pin-tag">Pino Físico: <b>GPIO 15</b></span>
        <div id="pillRelay_15" class="status-pill status-off">DESLIGADO (ABERTO)</div>
      </div>
      <div class="btn-group">
        <button class="btn btn-green" id="btnRelay_15" onclick="togglePin(15)">Acionar Relé</button>
        <button class="btn btn-secondary" onclick="pulsePin(15, 1000)">Pulso 1s</button>
      </div>
    </div>
    ...
    <div class="hero-card" id="cardBoot">
      ...
      <p style="font-size:0.78rem; color:var(--text-muted); margin-top:8px;">Pressione o botão IO0/BOOT na placa para ver a leitura mudar em tempo real.</p>
    </div>

  </div>

  <div class="section-title">
    <span>2. Mapeamento & Controle de GPIOs de Expansão</span>
  </div>
    ...
    <div class="modal">
      ...
      <label class="radio-option">
        ...
        <div>
          ...
          <div style="font-size:0.75rem; color:#94a3b8;">Permite ligar e desligar cargas ou atuadores</div>
        </div>
      </label>
  ...
  <footer>
    ROBOBUILDERS RB2554 (ESP-32_RELAY_30A_X4_V1.1) • Teste e Diagnóstico de Hardware
  </footer>

  <script>
    const relayPins = [12, 13, 14, 15];
    // GPIOs de expansão (LED=5, Relés=12,13,14,15, Boot=0)
    const gpioList = [2, 4, 16, 17, 18, 19, 21, 22, 23, 25, 26, 27, 32, 33, 34, 35];
    let pinConfigs = {};
        ...
        .then(function(data) {
          ...
          if (data.pins) {
            data.pins.forEach(function(p) {
              ...
              // 4 Relés de Potência (GPIO 12, 13, 14, 15)
              if (relayPins.indexOf(p.pin) !== -1) {
                var cardR = document.getElementById('cardRelay_' + p.pin);
                var pillR = document.getElementById('pillRelay_' + p.pin);
                var btnR = document.getElementById('btnRelay_' + p.pin);
                if (cardR && pillR && btnR) {
                  if (p.val === 1) {
                    cardR.className = 'hero-card active';
                    pillR.className = 'status-pill status-on';
                    pillR.innerText = 'LIGADO (FECHADO)';
                    btnR.className = 'btn btn-red';
                    btnR.innerText = 'Desligar Relé';
                  } else {
                    cardR.className = 'hero-card';
                    pillR.className = 'status-pill status-off';
                    pillR.innerText = 'DESLIGADO (ABERTO)';
                    btnR.className = 'btn btn-green';
                    btnR.innerText = 'Acionar Relé';
                  }
                }
              }
