...
// ==========================================
const char* AP_SSID = "ROBOBUILDERS-RB2554";
...
// ==========================================
// MAPEAMENTO DAS GPIOS DA PLACA RB2554 (ESP-32_RELAY_30A_X4_V1.1)
// ==========================================
enum PinModeType {
  PIN_MODE_OUTPUT = 0,
  PIN_MODE_INPUT  = 1
...
// Pinos dos 4 Relés de Potência (30A)
const uint8_t RELAY_PINS[] = { 12, 13, 14, 15 };
const size_t RELAY_COUNT = sizeof(RELAY_PINS) / sizeof(RELAY_PINS[0]);

// Lista completa de pinos mapeados
GPIOPin pins[] = {
  // Pinos Principais Onboard da RB2554
  { 5,  PIN_MODE_OUTPUT, 0, 0 }, // LED Onboard (GPIO 5) - Indicador de Status
  { 12, PIN_MODE_OUTPUT, 0, 0 }, // Relé 1 - 30A (GPIO 12)
  { 13, PIN_MODE_OUTPUT, 0, 0 }, // Relé 2 - 30A (GPIO 13)
  { 14, PIN_MODE_OUTPUT, 0, 0 }, // Relé 3 - 30A (GPIO 14)
  { 15, PIN_MODE_OUTPUT, 0, 0 }, // Relé 4 - 30A (GPIO 15)
  { 0,  PIN_MODE_INPUT,  1, 0 }, // Botão Boot / IO0 (GPIO 0) - Entrada com Pull-Up

  // Saídas / Entradas Digitais de Expansão para Teste
  ...
  // Pinos Apenas Entrada (ADC / Sensores)
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println("  ROBOBUILDERS RB2554 - TESTE DE PLACA    ");
  Serial.println("  Modelo: ESP-32_RELAY_30A_X4_V1.1        ");
  Serial.println("  LED Onboard: GPIO 5                    ");
  Serial.println("  Relé 1:      GPIO 12 (30A)              ");
  Serial.println("  Relé 2:      GPIO 13 (30A)              ");
  Serial.println("  Relé 3:      GPIO 14 (30A)              ");
  Serial.println("  Relé 4:      GPIO 15 (30A)              ");
...
void handleRelays() {
  ...
  for (size_t i = 0; i < PIN_COUNT; i++) {
    for (size_t r = 0; r < RELAY_COUNT; r++) {
      if (pins[i].pin == RELAY_PINS[r] && pins[i].mode == PIN_MODE_OUTPUT) {
        pins[i].state = val;
        pins[i].pulseEnd = 0;
