...
// ==========================================
const char* AP_SSID = "ROBOBUILDERS-RB0718";
...
// ==========================================
// MAPEAMENTO DAS GPIOS - ROBOBUILDERS RB0718
// ESP32 Dev Kit (Type-C)
// ==========================================
enum PinModeType {
  PIN_MODE_OUTPUT = 0,
  PIN_MODE_INPUT  = 1
...
// LED Built-in onboard no ESP32 DevKit padrão (GPIO 2)
#ifndef LED_BUILTIN
  #define LED_BUILTIN 2
#endif

// Lista completa de pinos do ESP32 DevKit (RB0718)
GPIOPin pins[] = {
  // Pinos Onboard
  { 2,  PIN_MODE_OUTPUT, 0, 0 }, // LED Built-in (Azul Onboard - GPIO 2)
  { 0,  PIN_MODE_INPUT,  1, 0 }, // Botão BOOT / IO0 (GPIO 0) - Pull-Up

  // Saídas / Entradas Digitais Configuráveis do DevKit
  ...
  // Pinos Apenas de Entrada (Input-Only / ADC / Sensores)
  { 34, PIN_MODE_INPUT,  0, 0 },
  { 35, PIN_MODE_INPUT,  0, 0 },
  { 36, PIN_MODE_INPUT,  0, 0 }, // SENSOR_VP
  { 39, PIN_MODE_INPUT,  0, 0 }  // SENSOR_VN
};
...
void setup() {
  ...
  Serial.println("\n==========================================");
  Serial.println("  ROBOBUILDERS RB0718 - ESP32 DEV KIT     ");
  Serial.println("  Firmware de Diagnóstico e Controle Web   ");
  Serial.println("  LED Built-in: GPIO 2                    ");
  Serial.println("  Botão Boot:   GPIO 0 (IO0)              ");
  Serial.printf("  Total de GPIOs Mapeadas: %d             \n", (int)PIN_COUNT);
  Serial.println("==========================================");
  ...
  Serial.printf("[Wi-Fi] Ponto de Acesso Aberto: %s\n", AP_SSID);
  Serial.printf("[Wi-Fi] IP de Acesso: http://%s\n", WiFi.softAPIP().toString().c_str());
  ...
  // Captive Portal Handlers (Android, iOS, Windows)
...
void setupGPIOs() {
  for (size_t i = 0; i < PIN_COUNT; i++) {
    ...
    if (pins[i].mode == PIN_MODE_OUTPUT) {
      ...
      // Pinos 34, 35, 36, 39 não possuem pull-up interno no hardware ESP32
...
void handleMode() {
  ...
  // Pinos 34, 35, 36 e 39 são apenas de entrada no ESP32
